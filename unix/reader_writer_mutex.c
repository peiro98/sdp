...
typedef struct rw_s
{
    long zero_us;
    int nr;
    pthread_mutex_t me_r;
    pthread_mutex_t me_w;
    pthread_mutex_t w;
} rw_t;
...
rw_t rw = {0L, 0, PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER};
...
void *reader(void *arg)
{
    ...
    wait_random_ns(50000);

    printf("[+%8ld] reader %2d: trying to read...\n", get_time_us() - rw.zero_us, i);

    // wait for the 'me_r' mutex
    pthread_mutex_lock(&rw.me_r);
    rw.nr++;
    if (rw.nr == 1)
    {
        // wait for the 'w' mutex
        pthread_mutex_lock(&rw.w);
    }
    pthread_mutex_unlock(&rw.me_r);
    ...
    if (rw.nr == 0)
    {
        // the 'w' mutex is released only when all the readers completed the 'reading' phase
...
void *writer(void *arg)
{
    ...
    wait_random_ns(500000);

    printf("[+%8ld] writer %2d: trying to write...\n", get_time_us() - rw.zero_us, i);

    pthread_mutex_lock(&rw.me_w);
    pthread_mutex_lock(&rw.w);
